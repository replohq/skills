...
- URL prefixes like `en-eu` are site routing conventions; they only affect prices after you map them to a concrete `CountryCode` and pass `country` on the loaders. Shopify serves those prefixes lowercase (`/en-eu/...`). Do not gate them with a case-sensitive regex — `getLocaleFromPath` from `@replohq/sdk/routing/locale` matches case-insensitively.
